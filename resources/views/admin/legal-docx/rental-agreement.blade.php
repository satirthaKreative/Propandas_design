...
<style>
	...
	.backend-form-modal .modal-body h3 {
	    font-size: 24px;
	    color: #18130d;
	...
	.backend-form-modal .modal-body input:not([type="submit"]), select, textarea {
	    display: block;
	    width: 100%;
	    height: 40px ;
	...
	.backend-form-modal .form-group label {
	    font-size: 14px;
	    margin-bottom: 7px;
	}

	.backend-form-modal textarea {
	    border-radius: 0px;
	}

	.backend-form-modal .form-control {
	    outline: none !important;
	    box-shadow: none;
	    font-size:14px;
	}

	.backend-form-modal .low-height-text{
	    height:55px;
	}

	.backend-form-modal .mdm-height-text{
	    height:65px;
	}

	.backend-form-modal .big-height-text{
	    height:75px;
	}

	.backend-form-modal .modal-body .info-btn {
	    background: #279ce0;
	    color: #fff;
	    padding: 6px 25px;
	    font-size: 14px;
	...
	.backend-form-modal .modal-body input[type=submit] {
	    display: block;
	    width: 100%;
...
<section class="content">
    	...
    	<div class="col-md-12">
    	    <div class="box">
    	        <div class="box-header">
    	            <h3 class="box-title">Legal Document Table</h3> <span class="float-right-btn"><a href="javascript: ;" onclick="legal_doc_modal()" class="btn btn-sm btn-success text-white">Add Legal Documents</a></span>
    	        </div><!-- /.box-header -->
    	        <div class="box-body">
    	            <table class="table table-bordered table-striped table-reponsive" id="legal-docx-value-show">
                        ...
                        <tr >
                            <td colspan="7" class="text-info"><center><i class="fa fa-spinner"></i> Loading legal documents</center></td>
                        </tr>
    ...
    <!-- modal -->
<div class="modal fade  backend-form-modal" id="rental-agreement-modal" >
         ...
         <div class="modal-content">
            ...
            <div class="modal-body ">
               <div class="center-part">
                  ...
                  <form id="my-rental-form-submit">
                     <div class="body-form">
                        <div class="form-group">
                        <label>Category Name </label> 
                         <select name="category_name" class="form-control" onchange="icon_field_manipulate()" id="legal-category-id">
                         	<option value="">Choose a category</option>
                         </select>
                     </div>
                     <div class="form-group" id="category-icon-id">
                        <label>Category Icon</label> 
                       <select class="form-control" id="category-icons-define-id" name="category_icons">
                         <option value="">Choose one category icon</option>
                     ...
                     <div class="form-group">
                        <label>Title</label> 
                        <textarea class="form-control low-height-text" name="legal_title"></textarea>                        
                     </div>

                     <div class="form-group">
                        <label>Short Description</label> 
                        <textarea class="form-control mdm-height-text" name="short_description"></textarea>                        
                     </div>

                     <div class="form-group">
                        <label>Document Type</label> 
                       <select class="form-control" name="document_type" id="document-type-to-file-id" onchange="document_type_to_file()">
                     ...
                     <div class="form-group">
                        ...
                        <textarea class="form-control big-height-text" name="full_description"></textarea>                        
                     </div>

                      <div class="form-group"  id="agreement-file-id">
                        <label>File upload</label> 
                        <input type="file" name="filename2" id="file-value-id">                        
                     </div>
                     
                     </div>

                     <input type="submit" name="" value="Submit " class="info-btn">
                     
                  </form>
                  <p id="return-msg-show"></p>
               </div>
            </div>
         </div>
      </div>
      ...
      <div class="modal-content">
         ...
         <div class="modal-body text-center">
            <div class="center-part">
               ...
               <p>
                  <span><a href="#" class="short-anchr" id="delete-confirm-btn-id">Yes</a></span>
         ...
         <div class="modal-content">
            ...
            <div class="modal-body contact-modal-design text-center">
               <div class="center-part">
                  ...
                  <h4>Your legal document data save to propandas database</h4>
               </div>
            </div>
         </div>
      </div>
...
<script>
	$(function(){
		category_load_for_icons();
		load_rental_agreement();
		$("form#my-rental-form-submit").submit(function(e){
			e.preventDefault();
			...
			$.ajax({
				...
				success:function(response){
				    if(response == "file type error")
				    {
				    	class_type_msg = "text-danger";
				    	i_class = '<i class="fa fa-times"></i>';
				    	show_return_msg = "Select a valid type of file";
				    }
				    else if(response == "success")
				    {
				    	class_type_msg = "text-success";
				    	i_class = '<i class="fa fa-check"></i>';
				    	show_return_msg = "Successfully inserted your data";
				    }
				    else if(response == "error")
				    {
				    	class_type_msg = "text-danger";
				    	i_class = '<i class="fa fa-times"></i>';
				    	show_return_msg = "Something goes wrong! try again later";
				    }
				    else
				    {
				    	class_type_msg = "text-danger";
				    	i_class = '<i class="fa fa-times"></i>';
				    	show_return_msg = "Server got an error! Try to fixing server";
				    }
				    $("#return-msg-show").html("<span class="+class_type_msg+">"+i_class+" "+show_return_msg+"</span>").fadeIn().delay(3000).fadeOut('slow');
				    setTimeout(function(){ load_rental_agreement(); $("#rental-agreement-modal").modal('hide'); }, 3000);
				}
			});
		});
	});
	// show legal doc modal
	function legal_doc_modal()
	{
		$("#rental-agreement-modal").modal('show');
	}
	// type wish file field change
	function document_type_to_file()
	{
		var doc_id = $("#document-type-to-file-id").val();

		if(doc_id == "Article")
		{
			$("#file-value-id").val(null);
			$("#agreement-file-id").css("display","none");
		}
		else if(doc_id == "Documents")
		{
			$("#agreement-file-id").css("display","block");
		}
	}

	// category show  for icons
	function category_load_for_icons()
	{
		$.ajax({
			url: '/category-load-for-icons',
			type: 'GET',
			dataType: 'JSON',
			success:  function(event)
			{
				$("#legal-category-id").html(event);
			},
	...
	// icon field manipulate

	function icon_field_manipulate()
	{
		var lC_id = $("#legal-category-id").val();
		$.ajax({
			url: '/icon-field-manipulate',
			type: 'GET',
			data: {cId: lC_id},
			dataType: 'JSON',
			success:  function(event)
			{
				...
				if(event == "yes")
				{
					$("#category-icons-define-id").val(null);
					$("#category-icon-id").css("display","none");
				}
				else 
				{
					$("#category-icon-id").css("display","block");
				}
	...
	function load_rental_agreement()
	{
		$.ajax({
			url: '/load-rental-agreement',
			type: 'GET',
			dataType: 'json',
			success:  function(event)
			{
				$("#legal-docx-value-show").html(event);
			},
			error:  function(event)
			{
	...
	function delete_legal_document(legal_id)
	{
		$("#delete-confirm-modal").modal('show');
		$("#delete-confirm-btn-id").attr('onclick','delete_confim_btn('+legal_id+')');
	}

	// deleting legal docx data
	function delete_confim_btn(legal_id)
	{
		$.ajax({
			...
			success: function(event)
			{
				...
				setTimeout(function(){ $("#success-save").modal('hide'); load_rental_agreement(); }, 3000);
			}, 
			error:  function(event)
			{
