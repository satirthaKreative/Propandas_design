<?php include ("inc/header.php") ?>
<!-- <section class="inner-banner">
   <div class="container">
      <h1>About Us</h1>
      <div class="breadcum-top">
         <ul class="breadcrumb">
            <li><a href="#">Home</a></li>
            <li>About Us </li>
         </ul>
      </div>
   </div>
   </section> -->
<section class="dshbord-theme">
   <div class="container">
      <div class="row">
         <div class="col-md-12">
            <h3 class="fs-title">Invoices</h3>
            <hr>
         </div>
         <div class="col-md-3">
            <div class="side-menu">
               <ul>
                  <li class="active"><a href="javascript:void(0)"><i class="fa fa-tachometer" aria-hidden="true"></i>Dashboard</a></li>
                  <li><a href="javascript:void(0)"><i class="fa fa-comments" aria-hidden="true"></i>Messages</a></li>
                  <li><a href="javascript:void(0)"><i class="fa fa-file-text" aria-hidden="true"></i>Documents</a></li>
                  <li><a href="javascript:void(0)"><i class="fa fa-briefcase" aria-hidden="true"></i>Posted Jobs</a></li>
                  <li><a href="javascript:void(0)"><i class="fa fa-file" aria-hidden="true"></i>Invoices</a></li>
                  <li><a href="javascript:void(0)"><i class="fa fa-exchange" aria-hidden="true"></i>Transactions</a></li>
                  <li><a href="javascript:void(0)"><i class="fa fa-comment" aria-hidden="true"></i>System Messages</a></li>
               </ul>
            </div>
         </div>
         <!-- end of col-md-3 -->
         <div class="col-md-9">
            <div class="dsbrd-content">           
         <div class="table-responsive theme-table prj-list-table">
         	<table class="table table-striped ">
         		<thead>
                        <tr>
                           <th>Project List</th>
                           <th>Project Name</th>
                           <th>Date</th>
                           <th>Status</th>
                           <th>Action</th>
                           <th>&nbsp;</th>
                        </tr>
                     </thead>
            <tbody>
          <tr>
            <td>01</td>
            <td>PROPAN441013</td>
            <td>June 30,2020</td>
            <td><span class="job-status completed">Completed</span></td>           
           
               <td>
                <a href="javascript:void(0)" class="shrt-btn short-link-btn ">Download</a>
              </td>
              <td> <a href="javascript:void(0)"   class="dlt-icn"><i class="fa fa-trash" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="Delete"></i></a></td>
             
            
          </tr>

          <tr>
           <td>02</td>
            <td>PROPAN441014</td>
            <td>July 01,2020</td>
            <td><span class="job-status proposal">Proposal</span></td>           
            <td>
                <a href="javascript:void(0)" class="shrt-btn short-link-btn ">Download</a>
              </td>
              <td> <a href="javascript:void(0)"   class="dlt-icn"><i class="fa fa-trash" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="Delete"></i></a></td>
          </tr>

          <tr>
            <td>03</td>
            <td>PROPAN441015</td>
            <td>July 05,2020</td>
            <td><span class="job-status initiated">Initiated</span></td>           
           <td>
                <a href="javascript:void(0)" class="shrt-btn short-link-btn ">Download</a>
              </td>
              <td> <a href="javascript:void(0)"   class="dlt-icn"><i class="fa fa-trash" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="Delete"></i></a></td>
          </tr>

          <tr>
            <td>04</td>
            <td>PROPAN441015</td>
            <td>July 06,2020</td>
            <td><span class="job-status in-progress">In Progress</span></td>           
           <td>
                <a href="javascript:void(0)" class="shrt-btn short-link-btn ">Download</a>
              </td>
              <td> <a href="javascript:void(0)"   class="dlt-icn"><i class="fa fa-trash" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="Delete"></i></a></td>
          </tr>

          <tr>
            <td>05</td>
            <td>PROPAN441020</td>
            <td>July 10 ,2020</td>
            <td><span class="job-status closed">Closed</span></td>           
           <td>
                <a href="javascript:void(0)" class="shrt-btn short-link-btn ">Download</a>
              </td>
              <td> <a href="javascript:void(0)"  class="dlt-icn"><i class="fa fa-trash" aria-hidden="true" data-toggle="tooltip" data-placement="top" title="Delete"></i></a></td>
          </tr>

          <!-- <tr>
            <td>05</td>
            <td>PROPAN441016</td>
            <td>April 10 ,2020</td>
            <td><span class="job-status closed">Closed</span></td>           
           <td>
              <a href="javascript:void(0)" class="shrt-btn short-link-btn ">Add</a>
              <a href="javascript:void(0)" class="shrt-btn short-link-btn danger-btn ">Cancle</a>
            </td>
          </tr>
 -->
          
            </tbody>

         	</table>
         </div>        
               
            </div>
         </div>
      </div>
   </div>
</section>
<!-- end of dshbord-theme -->
<?php include ("inc/footer.php") ?>



<script type="text/javascript">
  $(document).ready(function(){
     $('[data-toggle="tooltip"]').tooltip();     
   });
</script>